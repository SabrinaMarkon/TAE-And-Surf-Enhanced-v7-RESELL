...
if($userid != "")
   	{  // members only stuff!
		...
		echo "<p><b>Your referral url:</b><br><br>".$domain."/index.php?referid=".$userid."<p></b>";
echo "<p><b>Splash Page:</b><br><br>".$domain."/splashpage.php?ref=".$userid."<p></b>";
		...
		<textarea rows=4 cols=65 name="text"><a href="<? echo $domain."/index.php?referid=".$userid; ?>" target=_blank"><img src="<? echo $bannerurl; ?>" border=0></a></textarea><br>
		<br>
...
(for use on advertising websites or your own double opt-in subscribers only - do not send to purchased bulk email lists including \"blasters\")<br><br>";
    ...
    while ($line = mysql_fetch_array($result)) {
		...
		<textarea rows=10 cols=50 name="text"><? echo str_replace("%reflink%", $domain."/index.php?referid=".$userid, $line["text"]); ?></textarea><br>
		<br>
    	<?
    	}

        $query = "SELECT * FROM members where referid='".$userid."' and verified=1 ORDER BY joindate ASC";
		$result = mysql_query ($query);
